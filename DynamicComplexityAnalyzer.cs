...
namespace DGuo.Client.TileMatch.DesignerAlgo.Evaluation
{
    ...
    public class DynamicComplexityAnalyzer
    {
        ...
        /// <summary>
        /// 独立使用接口 - 用于测试和调试，包含完整的数据加载流程
        /// </summary>
        public static DynamicComplexityMetrics AnalyzeLevelIndependently(
            string levelName,
            int[] experienceMode,
            string playerType,
            int? colorCount = null,
            IAutoPlayStrategy strategy = null)
        {
            ...
            {
                // 重新创建完整的关卡数据（用于独立测试）
                var levelData = LoadLevelDataForTesting(levelName);
                if (levelData == null)
                {
                    return new DynamicComplexityMetrics { CompletionStatus = "Error - Failed to load level data" };
                }

                var tiles = CreateTileListFromLevelData(levelData);
                var algorithm = new RuleBasedAlgorithm();
                var availableColors = CreateAvailableColors(colorCount ?? levelData.ElementsPerLevel);
                algorithm.AssignTileTypes(tiles, experienceMode, availableColors);

                // 执行地形分析
                var terrainAnalyzer = new TerrainAnalyzer();
                var terrainAnalysis = terrainAnalyzer.AnalyzeTerrain(tiles, experienceMode);

                // 调用主要的动态分析方法
                return AnalyzeGameplayComplexity(tiles, experienceMode, terrainAnalysis, strategy);
            }
            catch (Exception ex)
            {
                Debug.LogError($"独立动态分析失败 {levelName}: {ex.Message}");
                return new DynamicComplexityMetrics
                {
                    CompletionStatus = $"Error - {ex.Message}",
                    GameCompleted = false
                };
            }
        }
        ...
        /// 用于测试的关卡数据加载
        /// </summary>
        private static LevelData LoadLevelDataForTesting(string levelName)
        {
            // 这里复用现有的关卡加载逻辑，仅用于独立测试
            // 主要流程不使用这个方法
            try
            {
                string jsonFileName;
                ...
                string jsonPath = System.IO.Path.Combine(Application.dataPath, "..", "Tools", "Config", "Json", "Levels", jsonFileName);
